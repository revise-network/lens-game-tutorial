...
export default class CactiController {
  ...
  nextCactusInterval: (null | number) = null;
  cacti: Cactus[] = [];
  ...
  constructor(ctx: any, cactiImages: any, scaleRatio: any, speed: number) {
    this.ctx = ctx;
  ...
  update(gameSpeed: number, frameTimeDelta: number) {
    if (!this.nextCactusInterval || this.nextCactusInterval <= 0) {
      this.createCactus();
      this.setNextCactusTime();
    }
    if (this.nextCactusInterval)
